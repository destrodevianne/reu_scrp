...
public class DrakeWarriors extends AbstractNpcAI
{
	...
	protected class RunTask implements Runnable
	{
		...
		public void run()
		{
			for (int groupId : _warriorsGroups.keySet())
			{
				...
				group._warrior.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(loc.getX(), loc.getY(), loc.getZ(), loc.getHeading()));
			}
		}
	}
	...
	{
		if ((npc == null) || (npc.getNpcId() != DRAKE_WARRIOR))
		{
			return null;
		}
		...
		{
			if ((npc.getNpcId() == DRAKE_WARRIOR) && npc.equals(group._warrior))
			{
				return group;
			}
		}
