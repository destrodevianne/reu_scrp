...
public class DrakeMages extends AbstractNpcAI
{
	...
	protected class RunTask implements Runnable
	{
		...
		public void run()
		{
			for (int groupId : _magesGroups.keySet())
			{
				...
				group._mage.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(loc.getX(), loc.getY(), loc.getZ(), loc.getHeading()));
			}
		}
	}
	...
	{
		if ((npc == null) || (npc.getNpcId() != DRAKE_MAGE))
		{
			return null;
		}
		...
		{
			if ((npc.getNpcId() == DRAKE_MAGE) && npc.equals(group._mage))
			{
				return group;
			}
		}
